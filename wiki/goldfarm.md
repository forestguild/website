...
В старом контенте очень много голды, да и получить ее не очень сложно.
...
Формат:
```markdown
### <Рейд> `<игроки> <сложность>` <количество> голды за <количество> минут
...
Пример:
```markdown
### Ульдуар `25 гер` 6531 голды за 10 минут
